...
- (void)addMenu:(NSDictionary *)menuData
{
    ...
    dic[@"menu_coupon_price"] = menuData[@"menu_coupon_price"];
            ...
            if (result)
            {
                ...
                if ([errorString length] == 0)
                {
                    ...
                    if (resultData && [resultData isKindOfClass:[NSDictionary class]])
                    {
                        ...
                        self.couponPrice = ((NSString *)resultData[@"original_total"]).floatValue;
                        self.cartList = [NSMutableArray array];
                        self.couponId = resultData[@"coupon_id"];
...
- (void)reCalculateTotal
{
    self.totalPrice = 0.0f;
    self.couponPrice = 0.0f;
    self.hasCouponPrice = NO;
    
    for (NSDictionary *menu in self.cartList)
    {
        NSString *menuPrice = menu[@"menu_price"];
        NSString *couponPrice = menu[@"menu_coupon_price"];
        
        if (couponPrice && couponPrice.length > 0)
        {
            self.totalPrice += couponPrice.floatValue;
            self.hasCouponPrice = YES;
        }
        else
        {
        ...
        self.couponPrice += menuPrice.floatValue;
    }
    
    // 如果用户已经选择了促销ID，则由服务器计算总价及促销价
    if (self.couponId && self.couponId.length > 0 && self.couponId.intValue > 0)
    {
        [self saveData];
    }
}
